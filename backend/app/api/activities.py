...
async def activity_stats(db: DbDep, user: UserDep) -> ActivityStatsResponse:
    ...
    func.coalesce guards a group whose every distance_m/moving_time_s/
    ascent_m happens to be NULL (moving_time_s alone, for an activity with
    no timestamps) from turning SUM's own NULL-for-empty-input into a NULL
    total for the whole bucket.
    """
    ...
    def _totals() -> tuple[Any, Any, Any, Any]:
        return (
            ...
            func.coalesce(func.sum(Activity.moving_time_s), 0.0).label("moving_time_s"),
    ...
    for row in month_rows:
        months_by_year.setdefault(int(row.year), []).append(
            ActivityMonthStats(
                ...
                moving_time_s=float(row.moving_time_s),
